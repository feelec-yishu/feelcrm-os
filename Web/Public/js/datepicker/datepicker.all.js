    ...
    event: function () {
      ...
      this.$container.on('click', '.c-datepicker-picker__shortcut', function () {
        ...
        if(days[0] == days[1]  && days[0]==10)
        {
          ...
          begin=moment(monthStartDate).format(_this.config.format);
          end=moment(monthEndDate).format(_this.config.format);
        }else{
          begin = moment().add(days[0], 'day').format(_this.config.format);
          end = moment().add(days[1], 'day').format(_this.config.format);
        }
