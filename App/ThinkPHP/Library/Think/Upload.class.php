...
class Upload {
    ...
    private function check($file)
    {
        ...
        /* 无效上传 */
        if (empty($file['name'])){
            $this->error = L('UPLOAD_FAIL1');
        }

        /* 检查是否合法上传 */
        if (!is_uploaded_file($file['tmp_name'])) {
            $this->error = L('UPLOAD_FAIL2');
            return false;
        }

        /* 检查文件大小 */
        if (!$this->checkSize($file['size'])) {
            $this->error = L('UPLOAD_FAIL3');
            return false;
        }
        ...
        if (!$this->checkMime($file['type'])) {
            $this->error =L('UPLOAD_FAIL4');
            return false;
        }

        /* 检查文件后缀 */
        if (!$this->checkExt($file['ext'])) {
            $this->error = L('UPLOAD_FAIL5');
            return false;
        }
    ...
    private function error($errorNo)
    {
        ...
        {
            case 1:
                $this->error = L('UPLOAD_ERROR1');
                break;
            case 2:
                $this->error = L('UPLOAD_ERROR2');
                break;
            case 3:
                $this->error = L('UPLOAD_ERROR3');
                break;
            case 4:
                $this->error = L('UPLOAD_ERROR4');
                break;
            case 6:
                $this->error = L('UPLOAD_ERROR5');
                break;
            case 7:
                $this->error = L('UPLOAD_ERROR6');
                break;
            default:
                $this->error = L('UPLOAD_FAIL1');
        }
