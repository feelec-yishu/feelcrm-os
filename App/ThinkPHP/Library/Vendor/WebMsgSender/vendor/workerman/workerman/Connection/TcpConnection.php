...
class TcpConnection extends ConnectionInterface
{
    ...
    public function doSslHandshake($socket){
	    ...
	    // Log file.
	    $log_file = LOG_PATH.'Socket/'.'Socket-'.date('Y-m-d').'.log';
	    ...
	    // Hidden error.
	    \set_error_handler(function($errno, $errstr, $file) use ($log_file)
	    {
		    if (!Worker::$daemonize)
		    {
//                Worker::safeEcho("SSL handshake error: $errstr \n");

			    \file_put_contents((string)$log_file, \date('Y-m-d H:i:s').' - SSL handshake error: '.$errstr."\n".'File - '.$file."\n\n"
				    ,FILE_APPEND | LOCK_EX);
		    }
	    });
