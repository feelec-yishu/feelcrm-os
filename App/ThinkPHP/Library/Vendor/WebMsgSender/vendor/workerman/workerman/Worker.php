...
//            static::$logFile = __DIR__ . '/../workerman.log';
	        ...
	        static::$logFile = LOG_PATH.'Socket/'.'Socket-'.date('Y-m-d').'.log';
        }
        $log_file = (string)static::$logFile;
        if (!\is_file($log_file))
        {
        	$dir_path = LOG_PATH.'Socket';
...
*/
//ThinkPHP 3.2.3 在Linux中启动Workerman - 开始
	    global $argv;
	    // Check argv;
	    $start_file = $argv[0];//yourfile.php
	    if (!isset($argv[2]))
	    {
	    ...
	    // Get command.
	    $command = trim($argv[2]);//start|stop|restart|reload|status|kill
	    $command2 = isset($argv[3]) ? $argv[3] : '';//-d
//ThinkPHP 3.2.3 在Linux中启动Workerman - 结束
    ...
    protected static function forkOneWorkerForLinux($worker)
    {
        ...
        elseif (0 === $pid) {
            ...
            Timer::delAll();
            static::setProcessTitle('FeelCrm V3 WorkerMan: worker process  ' . $worker->name . ' ' . $worker->getSocketName());
    ...
    public function listen()
    {
        ...
        if (!$this->_mainSocket) {
            ...
            if (!$this->_mainSocket) {
	            ...
	            $encode = mb_detect_encoding($errmsg, ["ASCII", "UTF-8", "GB2312", "GBK", "BIG5","EUC-CN"]);

	            $errmsg = iconv($encode,"UTF-8", $errmsg);
