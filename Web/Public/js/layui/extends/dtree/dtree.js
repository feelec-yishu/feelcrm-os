...
layui.define(['jquery','layer','form'], function(exports) {
	...
	var LI_NAV_CHILD = "dtree-nav-ul-sid", LI_NAV_ITEM = "dtree-nav-item",
		LI_DIV_ITEM = "dtree-nav-div", DTREEFONT = "dtreefont layui-icon", DTREEFONTSPECIAL="dtreefont-special",
	...
	var firstIconArray = {
		...
		"2" : {"open": "dtree-icon-wenjianjiazhankai", "close": "dtree-icon-weibiaoti5"}
	};
	...
	var nodeIconArray = {
        ...
        "1" : {"open": "layui-icon-down", "close": "layui-icon-right"}
	};
...
//			level: "level",			//层级
	...
	DTree.prototype.setting = function(options) {
        ...
        /** 自定义目录下拉选项数据 **/
        this.directoryOptions = $.extend(this.directoryOptions, this.options.directoryOptions) || this.directoryOptions;
    };
	...
	DTree.prototype.getToolbarDom = function(){
		...
		if(toolbarShow.length > 0){
			for(var i=0; i<toolbarShow.length; i++){
				...
				if(show == "add"){
					_this.toolbarMenu[defaultTool.addToolbar] = "<dd><a dtree-tool='"+defaultTool.addToolbar+"'><i class='"+DTREEFONT+" "+LI_DIV_TOOLBAR_ADD+"'></i>&nbsp;"+_this.toolbarStyle.create+"</a></dd>";
				}
				if(show == "edit"){
					_this.toolbarMenu[defaultTool.editToolbar] = "<dd><a dtree-tool='"+defaultTool.editToolbar+"'><i class='"+DTREEFONT+" "+LI_DIV_TOOLBAR_EDIT+"'></i>&nbsp;"+_this.toolbarStyle.editor+"</a></dd>";
				}
				if(show == "delete"){
					_this.toolbarMenu[defaultTool.delToolbar] = "<dd><a dtree-tool='"+defaultTool.delToolbar+"'><i class='"+DTREEFONT+" "+LI_DIV_TOOLBAR_DEL+"'></i>&nbsp;"+_this.toolbarStyle.delete+"</a></dd>";
				}
			}
		}
		...
		switch (name)
        {
			...
			case defaultTool.addToolbar:
				...
				var addNodeName = ['<div class="layui-form-item">',
					'<label class="layui-form-label">'+_this.toolbarStyle.title+'</label>',
					'<div class="layui-input-block f-input-par">',
				...
				var addNodeBtn = ['<div class="layui-form-item">',
					...
					'<button type="button" class="layui-btn layui-btn-normal btn-w100" lay-submit lay-filter="dtree_addNode_form">'+_this.toolbarStyle.submit+'</button>',
					'</div>',
					'</div>'].join('');
				//2. 用户自定义的节点内容
			...
			case defaultTool.editToolbar:
                ...
                //拼接上级目录下拉选项
                var directoryOptionsData = _this.directoryOptions.data;
                var directoryOptions = "";
                var defaultDirectoryId = _this.directoryOptions.value ? _this.directoryOptions.value : "";
                for(var key in optionsData)
                {
                    if(defaultDirectoryId == directoryOptionsData[key])
                    {
                        directoryOptions += "<option value='"+key+"' selected>"+directoryOptionsData[key]+"</option>";
                    }
                    else
                    {
                        directoryOptions += "<option value='"+key+"'>"+directoryOptionsData[key]+"</option>";
                    }
                }

                //1. 必须加载的节点内容
                var parentNode = ['<div class="layui-form-item">',
                    '<label class="layui-form-label" title="'+_this.directoryOptions.label+'">'+_this.directoryOptions.label+'</label>',
                    '<div class="layui-input-block f-input-par">',
...
*/
				...
				var editNodeBtn = ['<div class="layui-form-item">',
					'<div class="layui-input-block" style="margin-left:0px;text-align:center;">',
					'<button type="button" class="layui-btn layui-btn-normal btn-w100" lay-submit lay-filter="dtree_editNode_form">'+_this.toolbarStyle.submit+'</button>',
					'</div>',
					'</div>'].join('');

				var editNodeBar = ['<div class="'+TOOLBAR_TOOL+'"><form class="layui-form layui-form-pane" lay-filter="dtree_editNode_form">', parentNode, editNodeName];
				//2. 用户自定义的节点内容
				if(nodeBarContents != null && nodeBarContents.length > 0){
		...
		return{
			...
			select: function(nodeBarContents){
				...
				return ['<div class="layui-form-item">',
					'<label class="layui-form-label" title="'+nodeBarContents.label+'">'+nodeBarContents.label+'</label>',
					'<div class="layui-input-block f-input-par">',
					'<select name="'+nodeBarContents.name+'" lay-search>',
					options,
					'</select>',
					'</div>',
					'</div>'].join('');
		...
		_this.obj.prevAll('div#dtree_toolbar_'+_this.obj[0].id).on("click", "a[dtree-tool]", function(event) {
			...
			switch (tool) {
				case defaultTool.addToolbar:
					...
					layer.open({
						title: _this.toolbarStyle.create,
						type: 1,
				...
				case defaultTool.editToolbar:
					...
					layer.open({
						title: _this.toolbarStyle.editor,
						type: 1,
						area: _this.toolbarStyle.area,
						content: content,
						success: function(layero, index){
				...
				case defaultTool.delToolbar:
					layer.confirm(_this.toolbarStyle.delete_sure, {icon: 3,offset:['180px'], title:_this.toolbarStyle.delete}, function(index){
