...
class WXBizMsgCrypt
{
	...
	*/
	 ...
	 */
	public function EncryptMsg($sReplyMsg, $sTimeStamp, $sNonce,$type=1,&$sEncryptMsg)
	{
		$pc = new Prpcrypt($this->m_sEncodingAesKey);
		...
		if($type == 2)
		{
            $sEncryptMsg = $xmlparse->getContent($encrypt, $signature, $sTimeStamp, $sNonce);
        }
	...
	public function DecryptMsg($sMsgSignature, $sTimeStamp = null, $sNonce, $sPostData, &$sMsg)
	{
		...
		$pc = new Prpcrypt($this->m_sEncodingAesKey);
/*
*由于密文本身就是XML格式的明文加密，因此为了方便，此处过滤再从xml中提取密文
 ...
 */
		$result = $pc->decrypt($sPostData, $this->m_sCorpid);
