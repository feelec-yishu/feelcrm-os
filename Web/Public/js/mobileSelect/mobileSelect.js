...
(function() {
	...
	MobileSelect.prototype = {
		...
		init: function(config){
			var _this = this;
			_this.keyMap = config.keyMap ? config.keyMap : {id:'id', value:'value',color:'color',childs:'childs'};
			_this.checkDataType();
		...
		renderWheels: function(wheelsData, cancelBtnText, ensureBtnText){
			...
			//列
				...
				if(_this.jsonType){
					...
					//行
						tempHTML += '<li data-id="'+wheelsData[i].data[j][_this.keyMap.id]+'" style="color:'+wheelsData[i].data[j][_this.keyMap.color]+'">'+wheelsData[i].data[j][_this.keyMap.value]+'</li>';
					}
				}else{
					for(var j=0; j<wheelsData[i].data.length; j++){
					//行
		...
		addListenerWheel: function(theWheel, index){
			...
			theWheel.addEventListener('touchend', function () {
                ...
                /* Yishu修改 —— 选择项前后字体和颜色调整 */
				var id = _this.getCurValue()[0].id;

                $(theWheel).find("li[data-id='"+id+"']").css({'font-size':'4.5vw','color':'#48507d'}).siblings()
                    .css({'font-size':'3.4vw','color':'#9ba3b6'});

                $(theWheel).find("li[data-id='"+id+"']").prev().css({'font-size':'3.7vw','color':'#69758f'});

                $(theWheel).find("li[data-id='"+id+"']").next().css({'font-size':'3.7vw','color':'#69758f'});
			...
			//列
				(function (i) {
					...
					if(_this.wheel[i]){
						...
						//行
							tempHTML += '<li data-id="'+_this.displayJson[i][j][_this.keyMap.id]+'" style="color:'+_this.displayJson[i][j][_this.keyMap.color]+'">'+_this.displayJson[i][j][_this.keyMap.value]+'</li>';
						}
		...
		updateWheel: function(sliderIndex, data){
	    	...
	    	if(_this.cascade){
				...
				for(var j=0; j<data.length; j++){
					tempHTML += '<li data-id="'+data[j][_this.keyMap.id]+'" style="color:'+data[j][_this.keyMap.color]+'">'+data[j][_this.keyMap.value]+'</li>';
				}
